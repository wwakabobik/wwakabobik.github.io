    ...
    async def generate_image():
        ...
        for index, prompt in enumerate(prompts):
            midjourney_prompt = await ablt.chat(
                ...
                "Give me the prompt only, without any comments and descriptions. "
                "Just prompt output for midjourney.",
                stream=False,
            ).__anext__()
            dalle_prompt = await ablt.chat(
                bot_slug="maina",
                prompt=f"Please write a dalle3 prompt: '{prompt}'. "
                "Give me the prompt only, without any comments and descriptions. Just prompt output.",
                stream=False,
            ).__anext__()
...
.. code-block::
    ...
    TBD
    midjourney_variation_url = midjourney_wrapper(f"{midjourney_generated_image_url} {initial_midjourney_prompt})
...
.. code-block:: python
    ...
    async def main():
        ...
        leonardo_variations = await get_leonardo_variations(image_list)
        midjourney_variations = await get_midjourney_variations(image_list)
        pprint(dalle_variations, leonardo_variations, midjourney_variations)
